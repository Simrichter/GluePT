...
class GptModel(nn.Module):
    def __init__(self, cfg):
        ...
        #self.token_embedding.weight = self.emb_to_vocab.weight  # weight tying attempt
    ...
    def generate(self, x, max_length):
        for i in range(max_length):
            #print(Embeddings.decode(x.tolist()))
            logits, loss = self(x.view(1, -1)[:, -self.T:])
            logits = logits[:, -1, :]#.view(-1)
            #print(logits.shape)
            probs = func.softmax(logits, dim=-1)
            x_next = torch.argmax(probs)#torch.multinomial(probs, num_samples=1)#
            #print(Embeddings.decode([x_next.item()]))
            #print(Embeddings.decode([x_next.item()]))
            x = torch.cat((x, x_next.reshape(1)))  #TODO Cropping to max context_width (already done??)
        return x
